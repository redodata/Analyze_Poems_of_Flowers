...
function creationRanking(id) {
    ...
    $.get({
        ...
        success: function (data, status, xhr) {
            ...
            myChart.setOption(options, true);//设置true的目的是下次会清空上次的数据，重新渲染。
        },
...
function analyzeDynasty(id) {
    ...
    $.get({
        ...
        success: function (data, status, xhr) {
            ...
            //List<AuthorCount>
            for (var i=0; i< data.length; i++) {
                var dynastyCount  = data[i];
                options.xAxis.data.push(dynastyCount.dynasty);
                options.series[0].data.push(dynastyCount.count);
            }
            myChart.setOption(options, true);//设置true的目的是下次会清空上次的数据，重新渲染。
        },
        error: function (xhr, status, error) {
